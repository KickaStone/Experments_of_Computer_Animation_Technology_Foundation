...
namespace ParticlesGenerator{
	...
	ParticlesGenerator::ParticlesGenerator()
	{
		...
		mUpdateShader = new Shader(
			"E:\\Solutions\\Experments_of_Computer_Animation_Technology_Foundation\\test8\\res\\Shaders\\Update.vs", 
			"E:\\Solutions\\Experments_of_Computer_Animation_Technology_Foundation\\test8\\res\\Shaders\\Update.fs", 
			"E:\\Solutions\\Experments_of_Computer_Animation_Technology_Foundation\\test8\\res\\Shaders\\Update.gs", varyings, 5);
		//设置TransformFeedback缓存能够记录的顶点的数据类型

		mRenderShader = new Shader(
			"E:\\Solutions\\Experments_of_Computer_Animation_Technology_Foundation\\test8\\res\\Shaders\\Render.vs",
			"E:\\Solutions\\Experments_of_Computer_Animation_Technology_Foundation\\test8\\res\\Shaders\\Render.fs");
		//设置随机纹理
		InitRandomTexture(512);
		mSparkTexture.loadTexture("E:\\Solutions\\Experments_of_Computer_Animation_Technology_Foundation\\test8\\res\\Textures\\water.bmp");
		mRenderShader->use();
	...
	bool ParticlesGenerator::InitGenerator()
	{
	...
	void ParticlesGenerator::Render(float frametimeMills, glm::mat4& worldMatrix,
	...
	void ParticlesGenerator::UpdateParticles(float frametimeMills)
	{
		mUpdateShader->use();
	...
	void ParticlesGenerator::RenderParticles(glm::mat4& worldMatrix,
		glm::mat4& viewMatrix, glm::mat4& projectMatrix)
	{
	...
	void ParticlesGenerator::InitRandomTexture(unsigned int size)
	{
		srand(time(NULL));
	...
	void ParticlesGenerator::GenInitLocation(WaterParticle particles[], int nums) {
		srand(time(NULL));
